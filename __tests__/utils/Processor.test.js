...
describe('JSON2React', () => {
	...
	it('check serialized', () => {
		expect(html).toBe('bannercontentskewpanelgrouppanelrowcollinkrowcolheadlinetextrowcolheadlinetextrowcolheadline');
	});
	it('check single elements', () => {
		expect(tree[0].name).toBe(Constants.Element.Banner);
		expect(tree[0].children[0].name).toBe(Constants.Element.Content);
	});
});
